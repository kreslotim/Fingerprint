...
public class FingerprintCopy {
    ...
    public static boolean findBoolean(boolean[][] image, int row, int col) {
        ...
        return (row >= 0) && (row <= image.length - 1) && (col >= 0) && (col <= image[row].length - 1) && image[row][col];
    }
            ...
            for (int i = 0 ; i < image.length ; i++) {
                for (int j = 0 ; j < image[0].length ; j++) {
                    if ((image[i][j]) && (blackNeighbours(getNeighbours(pixRes,i,j)) > 0)
                            && !(i < row - distance || i > row + distance || j < col - distance || j > col + distance)) {
                        if (!pixRes[i][j]) {
                            pixRes[i][j] = true;
                            pixChanged = true;
                        }
    ...
    public static boolean match(List<int[]> minutiae1, List<int[]> minutiae2) {
        ...
        for (int[] m1 : minutiae1) {
            for (int[] m2 : minutiae2) {
                ...
                for (int rotTest = rotation - MATCH_ANGLE_OFFSET ; rotTest <= rotation + MATCH_ANGLE_OFFSET; rotTest++) {

                    List<int[]> transformedList = applyTransformation(minutiae1, m2Row, m2Col, rowTranslation, colTranslation, rotTest);

                    mFound = matchingMinutiaeCount(minutiae2, transformedList, DISTANCE_THRESHOLD, ORIENTATION_THRESHOLD);
